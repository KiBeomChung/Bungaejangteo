...
public class UserProvider {
    ...
    public int checkExisttKakaoUser(String phoneNum) throws BaseException {
        try {
            return userDao.checkExisttKakaoUser(phoneNum);
        } catch (Exception exception) {
            throw new BaseException(DATABASE_ERROR);
        }
    }
    ...
    @Transactional
    public PostUserRes kakaologIn(PostUserReq postUserReq) throws BaseException {
        try {
            userDao.setKaKaoLinked(postUserReq);
            int userIdx = userDao.getUserIdx(postUserReq);
            String jwt = jwtService.createJwt(userIdx);
            return new PostUserRes(userIdx, jwt, true);
        } catch (Exception exception) {
